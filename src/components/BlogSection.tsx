...
/* ── Unique SVG illustrations per post ── */
function IllustrationPlacement() {
  return (
    <svg viewBox="0 0 480 260" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Trophy */}
      <rect x="210" y="170" width="60" height="12" rx="4" fill="#FF7A1A" opacity=".25" />
      <rect x="228" y="150" width="24" height="22" rx="2" fill="#FF9847" opacity=".4" />
      <path d="M200 90 Q200 60 240 60 Q280 60 280 90 Q280 125 240 135 Q200 125 200 90Z" fill="#FF7A1A" opacity=".7" />
      <path d="M200 90 Q185 90 183 108 Q181 126 200 128" stroke="#FF7A1A" strokeWidth="6" fill="none" strokeLinecap="round" />
      <path d="M280 90 Q295 90 297 108 Q299 126 280 128" stroke="#FF7A1A" strokeWidth="6" fill="none" strokeLinecap="round" />
      <circle cx="240" cy="100" r="18" fill="white" opacity=".5" />
      <path d="M231 100 l6 6 12-12" stroke="#FF7A1A" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
      {/* Stars */}
      {[[100,60],[360,50],[380,170],[90,180],[420,100]].map(([cx,cy],i)=>(
        <circle key={i} cx={cx} cy={cy} r={3+i%2*2} fill="#FF7A1A" opacity={0.18+i*0.06} />
      ))}
      {/* Floating badges */}
      <rect x="30" y="90" width="110" height="34" rx="10" fill="white" opacity=".7" />
      <circle cx="50" cy="107" r="10" fill="#FF7A1A" opacity=".4" />
      <rect x="66" y="100" width="60" height="7" rx="3" fill="#FF7A1A" opacity=".35" />
      <rect x="66" y="111" width="42" height="5" rx="2" fill="#FF7A1A" opacity=".2" />
      <rect x="340" y="120" width="110" height="34" rx="10" fill="white" opacity=".7" />
      <circle cx="360" cy="137" r="10" fill="#FF7A1A" opacity=".4" />
      <rect x="376" y="130" width="60" height="7" rx="3" fill="#FF7A1A" opacity=".35" />
      <rect x="376" y="141" width="42" height="5" rx="2" fill="#FF7A1A" opacity=".2" />
      {/* Confetti dots */}
      {[[140,40],[300,30],[420,60],[60,200],[400,210]].map(([x,y],i)=>(
        <rect key={i} x={x} y={y} width="8" height="8" rx="2" fill="#FF7A1A" opacity={0.15+i*0.05} transform={`rotate(${i*18} ${x+4} ${y+4})`} />
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 480 260" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      <path d="M200 160 Q170 155 165 130 Q160 105 180 95 Q175 75 195 70 Q205 55 225 58 Q240 45 260 55 Q278 50 285 68 Q305 70 308 90 Q325 100 318 125 Q315 150 290 158 Q270 175 240 172 Q215 178 200 160Z"
        fill="#1EC8E8" opacity=".15" stroke="#1EC8E8" strokeWidth="2" />
      {/* Neural nodes */}
      {[[240,85],[215,105],[265,105],[200,130],[240,128],[278,128],[215,152],[265,152]].map(([cx,cy],i)=>(
        <circle key={i} cx={cx} cy={cy} r="6" fill="#1EC8E8" opacity={0.5+i*0.05} />
      ))}
      {/* Connections */}
      {[
        ...
        <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#1EC8E8" strokeWidth="1.5" opacity=".35" />
      ))}
      {/* Clock */}
      <circle cx="370" cy="80" r="36" fill="white" opacity=".6" stroke="#1EC8E8" strokeWidth="2" />
      <line x1="370" y1="80" x2="370" y2="56" stroke="#1EC8E8" strokeWidth="2.5" strokeLinecap="round" />
      <line x1="370" y1="80" x2="388" y2="88" stroke="#0d1b3e" strokeWidth="2" strokeLinecap="round" opacity=".5" />
      <circle cx="370" cy="80" r="4" fill="#1EC8E8" />
      {/* Study calendar */}
      <rect x="50" y="80" width="90" height="80" rx="10" fill="white" opacity=".6" stroke="#1EC8E8" strokeWidth="1.5" />
      <rect x="50" y="80" width="90" height="22" rx="10" fill="#1EC8E8" opacity=".25" />
      {[0,1,2].map(row=>[0,1,2].map(col=>(
        <rect key={`${row}-${col}`} x={62+col*26} y={116+row*16} width="14" height="8" rx="2" fill="#1EC8E8" opacity={0.15+Math.random()*0.2} />
      )))}
      {/* Percentage */}
      <text x="240" y="215" textAnchor="middle" fontFamily="sans-serif" fontSize="22" fontWeight="800" fill="#1EC8E8" opacity=".35">6 Month Plan</text>
    </svg>
  );
}
...
function IllustrationCampus() {
  return (
    <svg viewBox="0 0 480 260" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Building */}
      <rect x="160" y="90" width="160" height="120" rx="4" fill="#5353C8" opacity=".15" />
      <rect x="160" y="70" width="160" height="24" rx="4" fill="#5353C8" opacity=".25" />
      <rect x="215" y="60" width="50" height="14" rx="2" fill="#5353C8" opacity=".3" />
      {/* Windows */}
      {[0,1,2,3].map(col=>[0,1,2].map(row=>(
        <rect key={`${col}-${row}`} x={175+col*35} y={104+row*30} width="20" height="18" rx="3"
          fill="#5353C8" opacity={0.12+col*0.04+row*0.03} />
      )))}
      {/* Door */}
      <rect x="222" y="178" width="36" height="32" rx="4" fill="#5353C8" opacity=".3" />
      {/* People */}
      {[[80,185],[100,180],[390,185],[410,182],[430,188]].map(([cx,cy],i)=>(
        <g key={i}>
          <circle cx={cx} cy={cy-22} r="9" fill="#5353C8" opacity=".25" />
          <rect x={cx-7} y={cy-13} width="14" height="18" rx="4" fill="#5353C8" opacity=".2" />
        </g>
      ))}
      {/* Company logos placeholder */}
      {[[50,80],[380,70],[420,130],[40,150]].map(([x,y],i)=>(
        <rect key={i} x={x} y={y} width="50" height="28" rx="8" fill="white" opacity=".6" stroke="#5353C8" strokeWidth="1" strokeOpacity=".3" />
      ))}
      {/* Floating tag */}
      <rect x="320" y="90" width="120" height="38" rx="10" fill="white" opacity=".75" />
      <rect x="334" y="102" width="40" height="7" rx="3" fill="#5353C8" opacity=".4" />
      <rect x="334" y="113" width="70" height="5" rx="2" fill="#5353C8" opacity=".2" />
      <text x="240" y="245" textAnchor="middle" fontFamily="sans-serif" fontSize="13" fill="#5353C8" opacity=".3" fontWeight="700">On-Campus Recruitment</text>
    </svg>
  );
}
...
const posts = [
  {
    ...
    color: "#FF7A1A",
    colorLight: "rgba(255,122,26,0.08)",
  ...
  },
  {
    ...
    color: "#1EC8E8",
    colorLight: "rgba(30,200,232,0.08)",
  ...
  },
  {
    ...
    color: "#5353C8",
    colorLight: "rgba(83,83,200,0.08)",
    Illustration: IllustrationCampus,
  },
];
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          <div>
            <p className="text-xs font-bold tracking-[3.5px] uppercase mb-2" style={{ color: "#FF7A1A" }}>
              Resources
            </p>
        ...
        >
          {/* Left — illustration */}
          <div
            className="relative flex items-center justify-center overflow-hidden min-h-[240px] md:min-h-0"
            style={{ background: post.colorLight, transition: "background 0.4s" }}
          >
            ...
            {/* Category pill */}
            <div
              className="absolute top-5 left-5 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest"
              style={{
                background: "white",
                color: post.color,
                boxShadow: `0 4px 16px ${post.color}22`,
              }}
            >
              {post.category}
            </div>
