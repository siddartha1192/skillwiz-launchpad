...
const branches = [
  { label: "Computer Science", short: "CS",   color: "#1EC8E8" },
  { label: "Electronics",      short: "ECE",  color: "#FF7A1A" },
  { label: "Electrical",       short: "EE",   color: "#a78bfa" },
  { label: "Mechanical",       short: "ME",   color: "#1EC8E8" },
  { label: "Civil",            short: "CE",   color: "#FF7A1A" },
];

const highlights = [
  { icon: Globe,     color: "#1EC8E8", title: "Anytime & Anywhere",           desc: "On your phone, access tests, notes, videos and forums. Learn at your own pace while we monitor and guide you." },
  { icon: Zap,       color: "#FF7A1A", title: "Continuous Assessment",         desc: "Improve your areas of weakness with ongoing micro-assessments that adapt as you progress." },
  { icon: Target,    color: "#a78bfa", title: "Most Economical",               desc: "Save money compared to expensive coaching institutes — and save time by preparing on campus. We come to you." },
  { icon: BookOpen,  color: "#1EC8E8", title: "Digital & Classroom Coaching",  desc: "Best of both worlds — cutting-edge digital tools combined with expert classroom guidance." },
  { icon: Brain,     color: "#FF7A1A", title: "Personalized Coaching",         desc: "One student at a time. Our AI algorithm creates a path built specifically for your strengths and gaps." },
  { icon: BarChart3, color: "#a78bfa", title: "Data · Insight · Analytics",    desc: "Deep analytics on every test, every question — so you always know exactly where to focus next." },
];
...
const steps = [
  { num: "01", color: "#1EC8E8", title: "Enroll & Onboard",                      desc: "Sign up, pick your branch, and get your personalised dashboard set up in minutes." },
  { num: "02", color: "#FF7A1A", title: "Measure & Baseline",                    desc: "Take a diagnostic test. Our AI maps your knowledge graph and identifies exact gaps." },
  { num: "03", color: "#a78bfa", title: "Attend Right Classes",                  desc: "Online + offline sessions targeted to your weak areas — no wasted time on what you already know." },
  { num: "04", color: "#1EC8E8", title: "Take Tests to Find Gaps",               desc: "Continuous mock tests reveal regressions early and track your improvement week over week." },
];
      ...
      <div className="sticky overflow-hidden" style={{ top: "15vh", height: "70vh" }}>
        ...
        <div className="absolute top-0 left-0 right-0 h-[3px] z-30">
          <div className="h-full transition-all duration-500"
            style={{ width: `${((active + 1) / steps.length) * 100}%`, background: `linear-gradient(90deg, #1EC8E8, ${s.color})` }} />
        </div>

        {/* Step counter */}
        <div className="absolute top-5 right-8 z-20 flex items-center gap-1.5">
          <span className="font-black text-2xl" style={{ color: s.color }}>{String(active + 1).padStart(2, "0")}</span>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-6 z-20">
          <p className="text-[10px] font-bold tracking-[3.5px] uppercase mb-4" style={{ color: "#FF7A1A" }}>How It Works</p>
          <div key={active} className="flex flex-col items-center text-center"
            ...
            <div className="text-7xl font-black mb-4 leading-none" style={{ color: `${s.color}25`, letterSpacing: "-0.04em" }}>{s.num}</div>
            ...
            <p style={{ fontSize: "clamp(1rem,1.4vw,1.15rem)", color: "rgba(255,255,255,0.62)", maxWidth: 520 }} className="leading-relaxed mb-6">
              {s.desc}
            </p>
            <div className="inline-flex items-center gap-3 px-6 py-3 rounded-2xl"
              style={{ background: "rgba(0,0,0,0.3)", border: `1.5px solid ${s.color}35`, backdropFilter: "blur(10px)" }}>
              <div className="w-2 h-2 rounded-full" style={{ background: s.color, boxShadow: `0 0 8px ${s.color}` }} />
              <span className="text-sm font-semibold" style={{ color: "rgba(255,255,255,0.6)" }}>Step {active + 1} of {steps.length}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 mt-7">
            {steps.map((st, i) => (
              <div key={i} className="rounded-full transition-all duration-300"
                style={{ width: active === i ? 24 : 7, height: 7, background: active === i ? st.color : "rgba(255,255,255,0.2)", boxShadow: active === i ? `0 0 8px ${st.color}` : "none" }} />
            ))}
          </div>
        </div>
      </div>
...
export default function SmartGate() {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div ref={hero.ref} className="relative z-10 flex-1 flex items-center px-6 md:px-16 pt-32 pb-20">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-6"
              style={{ background: "rgba(30,200,232,0.15)", color: "#1EC8E8", border: "1px solid rgba(30,200,232,0.3)" }}>
            ...
            <h1 className="font-black text-white leading-[1.0] mb-5"
              ...
              Smart<span style={{ color: "#1EC8E8" }}>GATE</span>
            </h1>
            ...
            <p className="text-white/55 leading-relaxed mb-10" style={{ ...rv(hero.visible, 130), fontSize: "clamp(0.95rem,1.2vw,1.1rem)", maxWidth: 500 }}>
      ...
      {/* ══ BRANCHES ══ */}
      <section className="py-20 px-6 md:px-16" style={{ background: "linear-gradient(135deg,#f0f9ff 0%,#ffffff 50%,#fff7f0 100%)" }}>
        <div className="max-w-6xl mx-auto" ref={branchSec.ref}>
          <div className="text-center mb-12" style={rv(branchSec.visible)}>
            <p className="text-xs font-bold tracking-[3.5px] uppercase mb-2" style={{ color: "#FF7A1A" }}>Branches We Cover</p>
            <h2 className="font-black leading-tight mb-3" style={{ fontSize: "clamp(2rem,3.5vw,3rem)", color: "#0d1b3e", letterSpacing: "-0.02em" }}>
              Prepare for <span style={{ color: "#1EC8E8" }}>Success</span>
            </h2>
            <p className="text-base mx-auto" style={{ color: "rgba(13,27,62,0.5)", maxWidth: 560 }}>
          ...
          <div className="mt-10 rounded-2xl p-6 text-center" style={{ background: "rgba(13,27,62,0.03)", border: "1.5px dashed rgba(13,27,62,0.1)" }}>
            <p className="text-sm" style={{ color: "rgba(13,27,62,0.45)" }}>
              You can learn more about what's covered in each branch. If you would like your branch here,{" "}
              <a href="/demo" className="font-semibold transition-colors hover:underline" style={{ color: "#1EC8E8" }}>contact us →</a>
            </p>
          </div>
        </div>
      </section>
      ...
      {/* ══ PROGRAM HIGHLIGHTS ══ */}
      <section className="py-24 px-6 md:px-16" style={{ background: "linear-gradient(135deg,#0d1b3e 0%,#0f2048 60%,#0d1b3e 100%)" }}>
        ...
        <div className="max-w-6xl mx-auto relative z-10" ref={hlSec.ref}>
          <div className="text-center mb-14" style={rv(hlSec.visible)}>
            <p className="text-xs font-bold tracking-[3.5px] uppercase mb-2" style={{ color: "#FF7A1A" }}>Program Highlights</p>
            <h2 className="font-black text-white leading-tight mb-4" style={{ fontSize: "clamp(2rem,3.5vw,3rem)", letterSpacing: "-0.02em" }}>
              Best of Technology, Teaching<br /><span style={{ color: "#1EC8E8" }}>& Mentoring — Combined</span>
            </h2>
            <p className="text-white/50 mx-auto" style={{ maxWidth: 520 }}>
            ...
            {highlights.map((h, i) => {
              ...
              return (
                <div key={i}
                  ...
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = `${h.color}50`; el.style.background = `${h.color}0d`; el.style.boxShadow = `0 20px 48px ${h.color}18`; }}
      ...
      <section className="py-24 px-6 md:px-16" style={{ background: "linear-gradient(135deg,#f0f9ff 0%,#ffffff 60%,#fff7f0 100%)" }}>
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center pb-6" ref={prepareSec.ref}>
          ...
          <div style={rv(prepareSec.visible, 0, "left")}>
            <p className="text-xs font-bold tracking-[3.5px] uppercase mb-3" style={{ color: "#FF7A1A" }}>Prepare for Success</p>
            <h2 className="font-black mb-5 leading-tight" style={{ fontSize: "clamp(2rem,3.5vw,3rem)", color: "#0d1b3e", letterSpacing: "-0.02em" }}>
              Everything You Need<br /><span style={{ color: "#1EC8E8" }}>to Crack GATE</span>
            </h2>
            <p className="text-base leading-relaxed mb-8" style={{ color: "rgba(13,27,62,0.55)" }}>
            ...
            <ul className="space-y-3 mb-10">
              {offerings.map((o, i) => (
                <li key={i} className="flex items-start gap-3" style={rv(prepareSec.visible, 80 + i * 50)}>
                  <CheckCircle2 size={18} className="shrink-0 mt-0.5" style={{ color: "#1EC8E8" }} />
                  <span className="text-sm font-medium" style={{ color: "rgba(13,27,62,0.7)" }}>{o}</span>
                </li>
              ))}
            </ul>
            <Link to="/demo"
          ...
          <div style={rv(prepareSec.visible, 150, "right")}>
            <div className="relative" style={{ borderRadius: "1.5rem" }}>
              ...
              {/* Floating badges — outside image flow, positioned relative to wrapper */}
              ...
              <div className="absolute -bottom-4 -right-4 px-4 py-3 rounded-2xl"
                style={{ background: "linear-gradient(135deg,#1EC8E8,#38d9f5)", boxShadow: "0 8px 28px rgba(30,200,232,0.4)" }}>
                <div className="text-2xl font-black text-white">AI-Powered</div>
                <div className="text-xs font-semibold text-white/75">Personalized Path</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden py-28 px-6 text-center"
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] rounded-full pointer-events-none"
          style={{ background: "radial-gradient(ellipse,rgba(255,122,26,0.06) 0%,transparent 70%)" }} />

        <div className="relative z-10 max-w-2xl mx-auto">
          <p className="text-xs font-bold tracking-[3.5px] uppercase mb-4" style={{ color: "#FF7A1A" }}>Start Today</p>
          <h2 className="font-black mb-5 leading-tight" style={{ fontSize: "clamp(2.4rem,5vw,4rem)", letterSpacing: "-0.03em", color: "#0d1b3e" }}>
            Ready to Crack<br /><span style={{ color: "#1EC8E8" }}>GATE?</span>
          </h2>
          <p className="text-lg leading-relaxed mb-10" style={{ color: "rgba(13,27,62,0.5)" }}>
